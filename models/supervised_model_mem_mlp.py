...
import numpy as np
from losses.contrastive_loss_multi_lv_feat_cat_proj import MultiLvPixelContrastLoss, MultiLvPixelContrastLossMem, MultiLvPixelContrastLossImgMem

from models.discriminator import Discriminator, DomainClassifier
from models.seg_model import EMA, DilateResUNet, DilateResUNetCLMem, DilateResUNetCLMemMLPPH, MultiProjectionHead, ResUNetDecoder
...
class TrainModel():
    ...
    def seg_model_backward(self):
        ...
        self.source_out_dict = self.seg_forward(self.source_images, labels=self.source_labels, hard_anchor_sampling=True, random_sampleing=False)
        ...
        if self.config['training']['contrast'] and self.iter > self.config['training']['contrast_start_step']:
            ...
            anchors_ = self.source_out_dict['anchors_']
            y_ = self.source_out_dict['y_']

            if self.iter > self.config['training']['mem_start_step']:
                if self.config['training']['is_image_mem']:
                    queue_, queue_y_ = self.get_emb_queue_from_imgmem()
                    loss_source_contrast = self.contrast_loss_imgmem(anchors_, y_, queue_, queue_y_, self.iter)
                else:
                ...
                loss_source_contrast = self.contrast_loss(anchors_, y_, self.iter)
    ...
    def init_nets(self):
        self.seg_net = DilateResUNetCLMemMLPPH(config=self.config)
        self.multi_lv_projs = MultiProjectionHead(self.config['seg_model']['enc_multi_lv_dims'])
        ...
        for net in self.nets:
            ...
            if net_class_name == "DilateResUNet":
                ...
                self.init_weights(net, init_type=self.config['seg_model']['init']["init_type"], a=self.config['seg_model']['init']["init_a"])
            elif net_class_name == "DilateResUNetCLMemMLPPH":
                self.init_weights(net, init_type=self.config['seg_model']['init']["init_type"], a=self.config['seg_model']['init']["init_a"])
    ...
    def get_emb_queue_from_imgmem(self):
        ...
        out_dict = self.seg_forward(pool_images, labels=pool_labels, hard_anchor_sampling=False, random_sampleing=True)

        # extract embeddings from projection feauture map
        # emb_queue_, emb_queue_y = self.anchor_sampling(out_dict['embeds'], pool_labels)
        emb_queue_, emb_queue_y = out_dict['anchors_'], out_dict['y_']
