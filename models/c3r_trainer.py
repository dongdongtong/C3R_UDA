...
import numpy as np
from losses.contrastive_loss_dtm_adaptation import PixelContrastLossImgMem, PixelContrastLossMem, PixelContrastLoss

from models.discriminator import Discriminator, DomainClassifier
from models.seg_model import EMA, DilateResUNet, DilateResUNetCLMem, MultiProjectionHead, ProjectionHead, ResUNetDecoder
from utils.utils import LambdaLROnStep, get_scheduled_ema_consistency, PolyLR
from losses import GANLoss, TaskLoss
from losses.soft_seg_loss import _SoftWeightedSoftmaxLoss
...
class AdaptationModel():
    def __init__(self, config, writer, logger, visual, logdir):
        ...
        self.ema_proj_head = EMA(self.proj_head_DP, self.config['training']['ema_net_momentum'])

        # losses
        self.task_loss = TaskLoss(num_classes=self.config['n_class'])
        self.soft_ce = _SoftWeightedSoftmaxLoss(num_classes=self.config['n_class'])
        ...
        self.source_segment_queue = torch.randn(config['n_class'], config['training']['segment_memory_size'], config['training']['proj_dim'], requires_grad=False)
        self.source_segment_queue = nn.functional.normalize(self.source_segment_queue, p=2, dim=2).cuda()
        self.source_segment_queue_ptr = torch.zeros(config['n_class'], dtype=torch.long, requires_grad=False)

        self.source_pixel_queue = torch.randn(config['n_class'], config['training']['pixel_memory_size'], config['training']['proj_dim'], requires_grad=False)
        self.source_pixel_queue = nn.functional.normalize(self.source_pixel_queue, p=2, dim=2).cuda()
        self.source_pixel_queue_ptr = torch.zeros(config['n_class'], dtype=torch.long, requires_grad=False)
    ...
    def seg_model_backward(self):
        ...
        if self.config['training']['lambda_s2t_seg'] > 0:
            ...
            # compute source aux seg loss
            if self.config['training']['lambda_aux_seg'] > 0:
                self.fake_T_seg_aux_up = F.interpolate(self.fake_T_out_dict['seg_aux'], (256, 256), mode="bilinear")
                loss_fake_T_aux_seg_ce, loss_fake_T_aux_seg_dc = self.task_loss(self.fake_T_seg_aux_up, self.source_labels)
            ...
            if self.config['training']["lambda_consis_pred"] > 0 and self.iter > self.config['training']["consis_pred_start_step"]:
                ...
                if self.config['training']['lambda_aux_seg']:
                    self.fake_S_seg_aux_up = F.interpolate(self.fake_S_out_dict['seg_aux'], (256, 256), mode="bilinear")
                    loss_aux_seg_consis_ce, loss_aux_seg_consis_dc = self.task_loss(self.fake_S_seg_aux_up, self.target_pseudo_label)
        ...
        # if self.config['training']['contrast']:
            # mem queue update
            with torch.no_grad():
                self.ema.update(self.seg_net_DP)
            
            # soft teacher strategy for unsupervised target image & fake_S image
            if self.config['training']['use_ema_fake_S_consis']:
                with torch.no_grad():
                    target_ema_out = self.ema.model(self.target_images)
                    self.ema_target_pseudo_label = self.get_pseodu_labels(target_ema_out['seg'])

                self.fake_S_seg_aux_up = F.interpolate(self.fake_S_out_dict['seg_aux'], (256, 256), mode="bilinear")
                loss_fake_S_ce = self.soft_ce(self.fake_S_out_dict["seg"], self.ema_target_pseudo_label, target_ema_out['seg'].detach())
                loss_fake_S_ce_aux = self.soft_ce(self.fake_S_seg_aux_up, self.ema_target_pseudo_label, target_ema_out['seg'].detach())

                lambda_consistency = get_scheduled_ema_consistency(
                    self.iter,
                    self.config['training']['train_iters'],
                    self.config['training']['lambda_ema_t_fake_S_consis'],
                    self.config['training']['contrast_start_step'])
                loss_G += lambda_consistency * loss_fake_S_ce
                loss_G += lambda_consistency * self.config['training']['lambda_aux_seg'] * loss_fake_S_ce_aux

                log_losses["ema_consist/loss_fake_S_ce"] = loss_fake_S_ce.detach()
                log_losses["ema_consist/loss_fake_S_ce_aux"] = loss_fake_S_ce_aux.detach()
            
            if self.iter > self.config['training']['contrast_start_step']:
                # mem queue update
                with torch.no_grad():
                    # self.ema.update(self.seg_net_DP)
                    self.ema_proj_head.update(self.proj_head_DP)
                    source_ema_out = self.ema.model(self.source_images)
                    source_keys = self.ema_proj_head.model(source_ema_out['multi_lv_feats'][-1])
                    self._dequeue_and_enqueue(source_keys, torch.argmax(self.source_labels, -1))

                source_embedding = self.proj_head_DP(self.source_out_dict['multi_lv_feats'][-1])
                target_embedding = self.proj_head_DP(self.target_out_dict['multi_lv_feats'][-1])
                target_pseudo_label = self.get_pseodu_labels(self.target_out_dict['seg'])

                # fake_T_embedding = self.fake_T_out_dict['embeds']
                # fake_S_embedding = self.fake_S_out_dict['embeds']

                # feats_dict = {
                #     "source_feats": source_embedding,
                #     "target_feats": target_embedding,
                #     # "fake_T_feats": fake_T_embedding,
                #     # "fake_S_feats": fake_S_embedding,
                # }

                lambda_s_cl = get_scheduled_ema_consistency(
                        self.iter,
                        self.config['training']['train_iters'],
                        self.config['training']['lambda_contrast'],
                        self.config['training']['contrast_start_step'])
                lambda_t_cl = get_scheduled_ema_consistency(
                    self.iter,
                    self.config['training']['train_iters'],
                    self.config['training']['target_lambda_contrast'],
                    self.config['training']['contrast_start_step'])
                if self.iter > self.config['training']['mem_start_step']:
                    if self.config['training']['use_segment_mem']:
                        queue = torch.cat([self.source_pixel_queue, self.source_segment_queue], dim=1)
                    else:
                        queue = self.source_pixel_queue
                    loss_source_contrast = self.contrast_loss_pixelmem(source_embedding, target_embedding, torch.argmax(self.source_labels, -1), torch.argmax(target_pseudo_label, -1), queue, self.iter)
                    source_contrast_loss, target_contrast_loss = loss_source_contrast
                    loss_G += (lambda_s_cl * source_contrast_loss + lambda_t_cl * target_contrast_loss)
                    log_losses["contrast/source_contrast_loss"] = source_contrast_loss.detach()
                    log_losses["contrast/target_contrast_loss"] = target_contrast_loss.detach()
                else:
                    loss_source_contrast = self.contrast_loss(source_embedding, target_embedding, torch.argmax(self.source_labels, -1), torch.argmax(target_pseudo_label, -1), self.iter)
                    if loss_source_contrast != 0:
                        loss_G += lambda_s_cl * loss_source_contrast
                        log_losses["contrast/contrast_loss"] = loss_source_contrast.detach()

                with torch.no_grad():
                    self.writer.add_scalar("contrast/keys_std_value_on_each_channel", 
                        torch.std(source_embedding.permute(1,0,2,3).reshape(self.config['training']['proj_dim'], -1), dim=1).abs().mean(), self.iter)
    ...
    def dec_backward(self):
        ...
        fake_T = self.dec_img_toT_forward(self.detach(self.source_out_dict['multi_lv_feats']))
        fake_S = self.dec_img_toS_forward(self.detach(self.target_out_dict['multi_lv_feats']))
    ...
    def pred_dis_backward(self):
        ...
        if self.config['training']['lambda_adv_p'] > 0:
            source_seg_is_real = self.dis_pred_forward(self.source_out_dict['seg'].detach())
            target_seg_is_fake = self.dis_pred_forward(self.target_out_dict['seg'].detach())

            loss_pred_dis = 0.5 * (self.gan_loss(target_seg_is_fake, False) + self.gan_loss(source_seg_is_real, True))

            log_losses["dis/loss_pred_dis"] = loss_pred_dis.detach()

            if self.config['training']['lambda_adv_aux_p'] > 0 and self.config['training']['lambda_aux_seg'] > 0:
                source_aux_seg_is_real = self.dis_aux_pred_forward(self.source_out_dict['seg_aux'].detach())
                target_aux_seg_is_fake = self.dis_aux_pred_forward(self.target_out_dict['seg_aux'].detach())
    ...
    def step(self):
        self.set_requires_grad([self.seg_net_DP], requires_grad=True)
        ...
        self.optimizer_proj_head.zero_grad()
        self.seg_model_backward()
        self.optimizer_seg.step()
        self.optimizer_proj_head.step()
        self.lr_scheduler_seg.step()  # but self.lr_scheduler_seg here doesn't include proj_head parameters
        self.lr_scheduler_proj_head.step()

        if self.config['training']['lambda_s2t_seg'] > 0:
            self.set_requires_grad([self.seg_net_DP], requires_grad=False)
        ...
        if self.iter > self.config['training']['start_adv_p']:
            ...
            self.lr_scheduler_pred_dis.step()
    ...
    def init_nets(self):
        ...
        self.proj_head = ProjectionHead(dim_in=512, project_dims=self.config['training']['proj_dim'])
        ...
        self.nets = [self.seg_net, self.proj_head, self.decoder_T, self.decoder_S, self.D_T, self.D_S, self.D_P, self.D_P_latent]
        ...
        for net in self.nets:
            ...
            elif net_class_name == "ProjectionHead":
                self.init_weights(net, init_type=self.config['seg_model']['init']["init_type"], a=self.config['seg_model']['init']["init_a"])
        ...
        whether_DP = False if torch.cuda.device_count() == 1 else True
        self.seg_net_DP = self.init_device(self.seg_net, gpu_id=0, whether_DP=whether_DP)
        self.proj_head_DP = self.init_device(self.proj_head, gpu_id=0, whether_DP=whether_DP)
        self.decoder_T_DP = self.init_device(self.decoder_T, gpu_id=0, whether_DP=whether_DP)
        self.decoder_S_DP = self.init_device(self.decoder_S, gpu_id=0, whether_DP=whether_DP)
        self.D_T_DP = self.init_device(self.D_T, gpu_id=0, whether_DP=whether_DP)
        self.D_S_DP = self.init_device(self.D_S, gpu_id=0, whether_DP=whether_DP)
        self.D_P_DP = self.init_device(self.D_P, gpu_id=0, whether_DP=whether_DP)
        self.D_P_latent_DP = self.init_device(self.D_P_latent, gpu_id=0, whether_DP=whether_DP)

        self.nets_DP = [self.seg_net_DP, self.proj_head_DP, self.decoder_T_DP, self.decoder_S_DP, self.D_T_DP, self.D_S_DP, self.D_P_DP, self.D_P_latent_DP]
    ...
    def init_optimizers(self):
        ...
        self.optimizer_seg = torch.optim.Adam(self.seg_net_DP.parameters(), **optimizer_seg_params)
        self.optimizer_proj_head = torch.optim.Adam(self.proj_head_DP.parameters(), **optimizer_seg_params)
        ...
        self.optimizers = [self.optimizer_seg, self.optimizer_decoder, self.optimizer_img_dis, self.optimizer_pred_dis]
    ...
    def init_lr_schedulers(self, epoch_batches):
        ...
        self.lr_scheduler_proj_head = torch.optim.lr_scheduler.LambdaLR(
            self.optimizer_proj_head, 
            lr_lambda=LambdaLROnStep(
                n_epoches * epoch_batches, 
                0, 
                lr_decay_start_step, 
        ...
        self.lr_schedulers = [self.lr_scheduler_seg, self.lr_scheduler_proj_head, self.lr_scheduler_decoder, self.lr_scheduler_img_dis, self.lr_scheduler_pred_dis]
    ...
    def _dequeue_and_enqueue(self, ema_keys, labels):
        ...
        for bs in range(batch_size):
            ...
            this_label_ids = [x for x in this_label_ids if x != 5 and (this_label == x).nonzero().shape[0] > 2]
            ...
            for lb in this_label_ids:
                ...
                # ptr = int(self.source_segment_queue_ptr[lb])
                ...
                ptr = int(self.source_pixel_queue_ptr[lb])

                if ptr + K >= self.config['training']['pixel_memory_size']:
                    self.source_pixel_queue[lb, -K:, :] = nn.functional.normalize(feat, p=2, dim=1)
                    self.source_pixel_queue_ptr[lb] = 0
                else:
                    self.source_pixel_queue[lb, ptr:ptr + K, :] = nn.functional.normalize(feat, p=2, dim=1)
                    self.source_pixel_queue_ptr[lb] = (self.source_pixel_queue_ptr[lb] + 1) % self.config['training']['pixel_memory_size']
    ...
    @torch.no_grad()
    def _dequeue_and_enqueue_source_dtm_from_hr_lv(self, ema_keys, labels):
        batch_size = ema_keys.shape[0]
        feat_dim = ema_keys.shape[1]

        # ds_labels = labels.unsqueeze(1).float().clone()
        # ds_labels = torch.nn.functional.interpolate(ds_labels,
        #                                          (ema_keys.shape[2], ema_keys.shape[3]), mode='nearest')
        # ds_labels = ds_labels.squeeze(1).long()

        # X = ema_keys.permute(0, 2, 3, 1)
        # X = X.contiguous().view(X.shape[0], -1, X.shape[-1])  # shape B*(H*W)*FeatDim

        for ii in range(batch_size):
            this_y_hat = labels[ii]
            # this_ds_y_hat = ds_labels[ii]
            this_classes = torch.unique(this_y_hat)
            this_classes = [x for x in this_classes if x != 5]
            this_classes = [x for x in this_classes if (this_y_hat == x).nonzero().shape[0] > 2]

            this_feat = ema_keys[ii].contiguous().view(feat_dim, -1)

            for cls_i, cls_id in enumerate(this_classes):

                this_y_hat = this_y_hat.contiguous().view(-1)
                idxs = (this_y_hat == cls_id).nonzero()
                ...
                num_pixel = idxs.shape[0]
                perm = torch.randperm(num_pixel)
                pixel_update_freq = self.pixel_update_freq_bg if cls_id == 0 else self.pixel_update_freq_fg
                K = min(num_pixel, pixel_update_freq)
                ptr = int(self.source_pixel_queue_ptr[cls_id])

                scaled_sr_indices = self._get_scaled_location(idxs[perm[:K]], 256, 256, 8)
                scaled_sr_indices = torch.unique(scaled_sr_indices)
                if scaled_sr_indices.shape[0] <= 1:
                    continue
                
                K = scaled_sr_indices.shape[0]
                feat = this_feat[:, scaled_sr_indices].squeeze()
                feat = torch.transpose(feat, 0, 1)
                ...
                # scaled_sr_indices = self._get_scaled_location(indices, 256, 256, 8)
                # scaled_sr_indices = np.unique(scaled_sr_indices)

                # K = scaled_sr_indices.shape[0]
                # feat = X[ii, scaled_sr_indices, :].squeeze(1)
                # ptr = int(self.source_pixel_queue_ptr[cls_id])

                if ptr + K >= self.config['training']['pixel_memory_size']:
                    self.source_pixel_queue[cls_id, -K:, :] = nn.functional.normalize(feat, p=2, dim=1)
                    self.source_pixel_queue_ptr[cls_id] = 0
                else:
                    self.source_pixel_queue[cls_id, ptr:ptr + K, :] = nn.functional.normalize(feat, p=2, dim=1)
                    self.source_pixel_queue_ptr[cls_id] = (self.source_pixel_queue_ptr[cls_id] + 1) % self.config['training']['pixel_memory_size']

    @torch.no_grad()
    def _dequeue_and_enqueue_target_dtm_sr_lv(self, ema_keys, labels):
        batch_size = ema_keys.shape[0]
        feat_dim = ema_keys.shape[1]
        ...
        for bs in range(batch_size):
            this_feat = ema_keys[bs].contiguous().view(feat_dim, -1)
            this_label = labels[bs]
            this_label_ids = torch.unique(this_label)
            this_label_ids = [x for x in this_label_ids if x != 5 and (this_label == x).nonzero().shape[0] > 2]

            for lb in this_label_ids:
                cur_cls_y_hat_mask = (this_label == lb)

                # compute distance transform map
                cur_cls_y_hat_mask_npy = cur_cls_y_hat_mask.cpu().numpy()
                dtm = distance(cur_cls_y_hat_mask_npy)
                in_out_perc_intensity = np.percentile(dtm[cur_cls_y_hat_mask_npy == 1], self.config['training']['target_dtm_perc'])
                ...
                max_views = self.config['training']['target_max_views_bg'] if lb == 0 else self.config['training']['target_max_views']
                n_sel_pixels_clas_id = min(n_indices, max_views)  # cls_id pixels selected in this image
                ...
                inside_indices = np.random.choice(inside_indices, size=num_indside_sel, replace=False)
                boundary_indices = np.random.choice(boundary_indices, size=num_boundary_sel, replace=False)
                indices = np.concatenate([inside_indices, boundary_indices]) if self.config['training']['target_boundary_sel'] else inside_indices
                ...
                ptr = int(self.source_pixel_queue_ptr[lb])
                K = indices.shape[0]
                feat = this_feat[:, indices].squeeze()
                feat = torch.transpose(feat, 0, 1)
    ...
    def get_emb_queue_from_imgmem(self):
        ...
        emb_queue_, emb_queue_y, _ = self.anchor_sampling(out_dict['embeds'], pool_labels)
    ...
    @torch.no_grad()
    def _dtm_anchor_sampling(self, keys, labels):
        labels = labels.unsqueeze(1).float().clone()
        labels = torch.nn.functional.interpolate(labels,
                                                 (keys.shape[2], keys.shape[3]), mode='nearest')
        labels = labels.squeeze(1).long()
        ...
        for ii in range(batch_size):
            ...
            for cls_i, cls_id in enumerate(this_classes):
                ...
                #     selected_embedding_indices_ = selected_embedding_indices
                # else:
                #     selected_embedding_indices_ = np.concatenate([selected_embedding_indices_, selected_embedding_indices])
    ...
    def visualization(self):
        ...
        if self.config['training']['use_ema_fake_S_consis'] and self.iter > self.config['training']['contrast_start_step']:
        # if self.config['training']['use_ema_fake_S_consis']:
            visual_t_list.append(('target_pseudo_seg', self.ema_target_pseudo_label.argmax(-1).data.cpu().numpy()))
        ...
        if self.config['training']['lambda_s2t_seg'] > 0:
            ...
            visual_s_list += [('fake_T', self.fake_T.cpu()), 
                              ('pred_seg_fake_T', self.fake_T_out_dict['seg'].softmax(1).argmax(1).data.cpu().numpy())
                            ]
            visual_t_list += [('fake_S', self.fake_S.cpu()), 
                              ('pred_seg_fake_S', self.fake_S_out_dict['seg'].softmax(1).argmax(1).data.cpu().numpy())
                            ]
    ...
    def save(self, model_dir):
        torch.save(self.seg_net_DP.module.state_dict(), os.path.join(model_dir, "base_model_" + str(self.iter) + ".pt"))
        torch.save(self.proj_head_DP.module.state_dict(), os.path.join(model_dir, "proj_head_" + str(self.iter) + ".pt"))
        torch.save(self.decoder_S_DP.module.state_dict(), os.path.join(model_dir, "decoder_S_" + str(self.iter) + ".pt"))
        torch.save(self.decoder_T_DP.module.state_dict(), os.path.join(model_dir, "decoder_T_" + str(self.iter) + ".pt"))
        torch.save(self.D_T_DP.module.state_dict(), os.path.join(model_dir, "D_T_" + str(self.iter) + ".pt"))
        torch.save(self.D_S_DP.module.state_dict(), os.path.join(model_dir, "D_S_" + str(self.iter) + ".pt"))
        torch.save(self.D_P_DP.module.state_dict(), os.path.join(model_dir, "D_P_" + str(self.iter) + ".pt"))
        torch.save(self.ema.model.module.state_dict(), os.path.join(model_dir, "base_model_ema_" + str(self.iter) + ".pt"))
    
    def save_valid_best(self, model_dir):
        torch.save(self.seg_net_DP.module.state_dict(), os.path.join(model_dir, "best_model.pt"))
        torch.save(self.proj_head_DP.module.state_dict(), os.path.join(model_dir, "proj_head.pt"))
        torch.save(self.decoder_S_DP.module.state_dict(), os.path.join(model_dir, "decoder_S.pt"))
        torch.save(self.decoder_T_DP.module.state_dict(), os.path.join(model_dir, "decoder_T.pt"))
        torch.save(self.D_T_DP.module.state_dict(), os.path.join(model_dir, "D_T.pt"))
        torch.save(self.D_S_DP.module.state_dict(), os.path.join(model_dir, "D_S.pt"))
        torch.save(self.D_P_DP.module.state_dict(), os.path.join(model_dir, "D_P.pt"))
        torch.save(self.ema.model.module.state_dict(), os.path.join(model_dir, "best_model_ema.pt"))
