...
import numpy as np
from losses.contrastive_loss import PixelContrastLossImgMem, PixelContrastLoss, PixelContrastLossMem
...
class TrainModel():
    def __init__(self, config, writer, logger, visual, logdir):
        ...
        self.proj_dims = [64, 96, 128, 256]
        self.segment_queue = torch.randn(config['n_class'], config['training']['segment_memory_size'], sum(self.proj_dims), requires_grad=False)
        ...
        self.pixel_queue = torch.randn(config['n_class'], config['training']['pixel_memory_size'], sum(self.proj_dims), requires_grad=False)
    ...
    def seg_model_backward(self):
        ...
        if self.config['training']['contrast'] and self.iter > self.config['training']['contrast_start_step']:
            loss_source_contrast = torch.Tensor([0]).cuda()
            # mem queue update
            ...
            seg_out_pred_dict = {0: "logits_hr", 1: "logits_mr", 2: "logits_mmr", 3: "logits_sr"}
            multi_lv_embeddings = self.multi_lv_projs_DP(self.source_out_dict['multi_lv_features'])
            ...
            if self.iter > self.config['training']['mem_start_step']:
                if self.config['training']['is_image_mem']:
                    queue_pool = self.get_emb_queue_from_imgmem()
                    for this_lv_index, (this_queue_, this_queue_y) in enumerate(queue_pool):
                        if this_lv_index == self.config['n_class'] - 1:
                            predict = torch.argmax(torch.softmax(self.source_out_dict[seg_out_pred_dict[this_lv_index]], 1), 1)
                            this_lv_imgmem_contrast_loss = self.contrast_loss_imgmem(multi_lv_embeddings[this_lv_index], torch.argmax(self.source_labels, -1), predict, this_queue_, this_queue_y, self.iter)
                            loss_source_contrast += this_lv_imgmem_contrast_loss
                            log_losses["contrast/{}_lv_contrast_loss".format(this_lv_index)] = this_lv_imgmem_contrast_loss.detach()
                else:
                ...
                # contrast_loss = self.contrast_loss(multi_lv_embeddings, multi_lv_embeddings_labels, self.iter)
                for this_lv_index, this_lv_embeddings in enumerate(multi_lv_embeddings):
                    if this_lv_index == self.config['n_class'] - 1:
                        predict = torch.argmax(torch.softmax(self.source_out_dict[seg_out_pred_dict[this_lv_index]], 1), 1)
                        this_lv_contrast_loss = self.contrast_loss(this_lv_embeddings, torch.argmax(self.source_labels, -1), predict, self.iter)
                        loss_source_contrast += this_lv_contrast_loss
                        log_losses["contrast/{}_lv_contrast_loss".format(this_lv_index)] = this_lv_contrast_loss.detach()
            ...
            #         torch.std(self.source_out_dict['keys'].permute(1,0,2,3).reshape(256, -1), dim=1).abs().mean(), self.iter)
    ...
    def step(self):
        ...
        self.optimizer_projs.zero_grad()
        self.seg_model_backward()
        self.optimizer_seg.step()
        self.optimizer_projs.step()
        self.lr_scheduler_seg.step()
        self.lr_scheduler_multi_projs.step()
    ...
    def seg_forward(self, input, labels=None, multi_lv_cat_proj=False):
        out_dict = self.seg_net_DP(input, labels, multi_lv_cat_proj)
        return out_dict
    ...
    def init_nets(self):
        # self.seg_net = DilateResUNetMultiLvSeg(n_channels=self.config["size_C"], n_classes=self.config["n_class"], 
    ...
    def init_lr_schedulers(self, epoch_batches):
        ...
        self.lr_scheduler_multi_projs = torch.optim.lr_scheduler.LambdaLR(
            self.optimizer_projs, 
            lr_lambda=LambdaLROnStep(
                n_epoches * epoch_batches, 
                0, 
                lr_decay_start_step, 
    ...
    def get_emb_queue_from_imgmem(self):
        ...
        queue_pool = []
        for this_lv_feats_proj in multi_lv_feats_proj:
            this_lv_emb_queue_, this_lv_emb_queue_y = self.anchor_sampling(this_lv_feats_proj, pool_labels)
            queue_pool.append((this_lv_emb_queue_, this_lv_emb_queue_y))
    ...
    def val_step(self, val_dataloader):
        ...
        for val_batch in val_dataloader:
            ...
            val_out_dict = self.seg_forward(val_images)
            # print()
            multi_lv_keys = self.multi_lv_projs_DP(val_out_dict)
            keys = multi_lv_keys[-1]
            batch_size = keys.shape[0]
            feat_dim = keys.shape[1]
